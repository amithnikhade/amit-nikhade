...
def database1():
   ...
   with conn:
      cursor=conn.cursor()
   cursor.execute('CREATE TABLE IF NOT EXISTS Student (username TEXT,password TEXT,email TEXT)')
   cursor.execute('INSERT INTO Student (username,password,emai) VALUES(?,?,?)',(username4,password4,email5,))
   conn.commit()
...
E1.config(text="Username",font='bold'*5)
#E1.pack()
